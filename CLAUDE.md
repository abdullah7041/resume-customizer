...
- **`strongMatches` evidence gate (2026-07-26)**: `strongMatches` is a bare `string[]` in both the JSON schema and Zod, so `STRONG_MATCH_EVIDENCE_RULE` in `netlify/lib/ai-contracts/contracts/index.js` is the ONLY definition of what earns one — it is injected into `buildMatchMessages` AND `buildMatchRealityCheckMessages` (NOT optimize, which has no such field). Without it every model tested (2.5-flash, 2.5/3.1/3.5-flash-lite, mistral-small-3.2) read the field as raw JD↔resume keyword overlap and failed in BOTH directions: keyword-stuffed skills lines were credited as strengths (`hard_skills 40/40` on a resume with zero ML evidence) while a demonstrated Power BI got no credit because the model pasted whole JD requirement bullets and dropped "Strong Power BI AND Tableau" wholesale when Tableau was absent. The rule fixes both by pinning **granularity** (one entry per individual skill in the JD's own short wording — never a requirement sentence; split compound "X AND/OR Y" requirements and judge each part alone) plus the **evidence-location test** (the term must appear in a work bullet / project / certification / coursework detail saying what the candidate DID with it; skills-list-, summary-, or headline-only mentions are unproven claims → `missingKeywords`, never a strength). Guarded by `npm run eval:match` (8/8 @ 100%) and a prompt-text test in `netlify/lib/__tests__/ai-contracts.test.js`. Do NOT weaken it to "score skills on demonstrated proficiency" — that phrasing constrains scoring, not the list, and is what was already there while the fixture failed.
- **Category score scale**: `MATCH_SCORING_RUBRIC` requires each `categoryScores` entry to set `max` to its rubric weight (40/30/15/15) and `score` to an integer `0..max`. The model otherwise mixes scales and emits `education: {score: 100, max: 15}` (eval `structure` hard-fail). Do NOT require the four category scores to sum to the overall `score` — they are decoupled on purpose (the models' own sums run 30+ points above their stated overall on partial matches, which would push those resumes into the 80+ "hireable today" band).
