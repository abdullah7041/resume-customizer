...
**AI Config**: `netlify/lib/ai-config.ts` centralizes defaults (`gpt-5-nano`, temp=1, max tokens 1-4096)
...
- **Validation**: Binary/base64 data detection prevents corrupted plainText display
  ```javascript
...
- **Fix**: Check `MainContent.jsx` validates data on load (see validation pattern above)
...
{
  model: "gpt-5-nano",
  temperature: 1, // make it lower if it work for better result
